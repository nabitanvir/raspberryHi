...
def main():
    ...
    chosen_dir = input("[PROMPT] Positive or negative recordings? (p/n): ")
    if chosen_dir == 'p':
        save_directory = config.POSITIVE_DIRECTORY
    elif chosen_dir == 'n':
        save_directory = config.NEGATIVE_DIRECTORY
    else:
        print("[INFO] Directory not chosen, terminating...")
        exit()

    num_recordings = int(input("# of recordings: "))
    
    print("[PROMPT] Press enter to start each recording\n")
    for i in range(num_recordings):
        input(f"[PROMPT] Press Enter to start recording {i + 1}/{num_recordings}")
        ...
        print(f"[INFO] Saved recording {i + 1} as {filepath}\n")

    print("[INFO] Recording session complete, terminating...")
